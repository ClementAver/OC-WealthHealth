...
describe("Given I am an user.", () => {
  ...
  describe("When I arrive on the application.", () => {
    ...
    describe("When I enter a first name in the corresponding input.", () => {
      ...
      test("If the data entered does not correspond to the input's expectations on submit, then I should be warned.", async () => {
    ...
    test("If I submit a valid form, then I should be informed that an employee has successfully been created.", async () => {
      const firstNameInput = screen.getByPlaceholderText("John");
      await userEvent.click(firstNameInput);
      await userEvent.keyboard("John");
      await userEvent.keyboard("{Enter}");
